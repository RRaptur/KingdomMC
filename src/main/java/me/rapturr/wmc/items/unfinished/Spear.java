...
public class Spear extends WesternItem {
    public Spear(String itemID, String displayName, boolean isStackable, boolean isShiny, ItemRarity rarity, Material mat, String desc, ItemType type, boolean hasActiveEffect) {
    ...
    private void throwTomahawk(Player player) {
        ...
        if (isReloaded) {
            ...
            as.getEquipment().setItemInMainHand(new ItemStack(Material.STICK));
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (!as.getLocation().add(0, 1, 0).getBlock().isPassable()) {
                        ...
                        isReloaded = true;
                        cancel();
                        as.getWorld().playEffect(as.getLocation(), Effect.SMOKE, 5);
                    }

                    if (distance >= 40) {
                        as.remove();
                        as.playEffect(EntityEffect.ENTITY_POOF);
                        cancel();
                        isReloaded = true;
                    }
