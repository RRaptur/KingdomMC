...
import me.rapturr.wmc.projectiles.snowballs.WhipProjectile;
...
public class Whip extends WesternItem {
    Cooldown cooldown = new Cooldown();

    public Whip(String itemID, String displayName, boolean isStackable, boolean isShiny, ItemRarity rarity, Material mat, String desc, ItemType type, boolean hasActiveEffect) {
    ...
    private void useWhip(Player player) {

        if (!cooldown.hasCooldownTimeLeft(player)) {
            WhipProjectile projectile = new WhipProjectile("WHIP_PROJECTILE");
            projectile.spawnSnowball(player.getEyeLocation(), player.getEyeLocation().getDirection().multiply(1.1), player);
            projectile.getSnowball().setItem(new ItemStack(Material.STONE_BUTTON));
            player.playSound(player.getLocation(), Sound.ENTITY_BLAZE_BURN, 1F, 0.5F);
            cooldown.setCooldown(player, 250L);
        }
